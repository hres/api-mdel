    ...
    public class MdelsJsonController : Controller
    {
        ...
        public ActionResult DownloadCSV(string dataType)
        {
            ...
            switch (dataType)
            {
                ...
                case ("company"):
                    var companies = dbConnection.GetAllCompany(string.Empty);
                ...
                case ("country"):
                    var countries = dbConnection.GetAllCountry("en", string.Empty);
                ...
                case ("establishment"):
                    var establishments = dbConnection.GetAllEstablishment(string.Empty);

                    if (establishments.Count > 0)
                    {
                        var json = JsonConvert.SerializeObject(establishments);
                        if (!string.IsNullOrWhiteSpace(json))
                        {
                            DataTable dt = JsonConvert.DeserializeObject<DataTable>(json);
                ...
                case ("province"):
                    var province = dbConnection.GetAllCountry("en", string.Empty);
